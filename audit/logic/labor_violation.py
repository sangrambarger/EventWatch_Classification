...
Thin binding: the decision lives in `regulatory_family.py`, which groups the event types that share a rule
...
from .regulatory_family import (
    LABOR_VIOLATION_EVENT_TYPE as EVENT_TYPE,
    LABOR_VIOLATION_SOURCE as SOURCE,
    decide_labor_violation,
)

try:  # pragma: no cover - only some families declare a shared field list
    from .regulatory_family import MUST_HAVE_FIELDS  # noqa: F401
...
def decide(fields: Mapping[str, object]) -> Decision:
    return decide_labor_violation(fields)
